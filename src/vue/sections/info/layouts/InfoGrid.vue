<template>
    <div class="row gx-5">
        <div v-for="item in props.items" class="col-12 col-md-6 item-container">
            <InfoItem :item="item" :highlighted-header="true" :small-description="false"
                :description-with-progress-bar="true"
                :icon-color-style="item['formattedPercentage'] ? 'transparent' : 'solid'" class="info-item" />
        </div>
    </div>
</template>
...
.item-container {
    @include generate-dynamic-styles-with-hash((xxxl: (min-height:5.0rem),
