<template>
    <div class="info-item">
        <!-- Icon -->
        <CircleIcon :src="item['imageIconUrl'] || item['faIcon'] || 'fa-regular fa-sticky-note'" :type="'standard'"
        ...
        <div class="info-item-content">
            ...
            <div v-if="highlightedHeader" class="info-item-content-header-highlight mb-1">
                ...
                <span class="info-item-progress-span text-1 text-muted fw-bold">{{ item['formattedPercentage'] ?? ''
                    }}</span>
            </div>
            ...
            <ProgressBar v-if="props.descriptionWithProgressBar && item['formattedPercentage']"
                :percentage="item['value']" :description="item['locales']['description']"
                :color="_getProgressBarColor(item)" class="mt-1" />
...
.info-item-icon {
    @include generate-dynamic-styles-with-hash((xxxl: (margin-right: 1rem),
