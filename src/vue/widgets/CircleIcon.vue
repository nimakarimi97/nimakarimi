<template>
    <div :class="_getCircleClassList()">
        <!-- If src is an image... -->
        <ImageView v-if="_isImage()" class="circle-icon-img" :src="props.src" :alt="'icon'" :style="_getStyle()" />

        <!-- Else... (font awesome icon stack) -->
        <div v-else class="circle-icon-fa-icon" :style="_getStyle()">
            <!-- Icon -->
...
@include create-size-variant((xxxl: 1.2rem, xxl: 1.15rem, xl:1.1rem, lg: 1.05rem, md: 0.95rem, sm: 0.9rem),
    standard);

@include create-size-variant((xxxl: 1rem, xxl: 1rem, lg: 0.9rem, md: 0.8rem, sm: 0.7rem),
    small);
</style>